<style>
     * {
        ...
        .hero {
            ...
            rgba(0, 0, 0, 0.616)), url("Images/gettyimages-469944715-612x612.jpg") var(--bg-image-center) fixed;
        }
...
<h1 class="main-headings center">
  ...
  <tr>
        ...
        <th>Patient Name</th>
        <th>Food customize</th>
    </tr>
    </thead>
    <tbody>
    <?php
$connection = mysqli_connect("localhost", "root", "", "patientpulse");
$query = "SELECT ID, Meals, Food, Ward, Room, `Order`, Name, Description FROM `food_order`"; // Enclose `Order` in backticks
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<td>" . $row["Ward"] . "</td>";
    echo "<td>" . $row["Room"] . "</td>";
    echo "<td>" . $row["Order"] . "</td>"; // Access 'Order' using $row["Order"]
    echo "<td>" . $row["Name"] . "</td>";
...
?>
  ...
  <a class='btn-sm' href='../PatientPulse/ManageFood.php'>Back to Order Page</a>
